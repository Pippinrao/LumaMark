...
- `codemirror-markdown-tables@1.0.0` dispatches an internal `table.format` transaction when loading non-canonical tables. At the table capability boundary, LumaMark rejects this passive formatting without user intent: non-canonical tables keep source verbatim and degrade to a plain source surface; only already-normalized tables mount the mature interaction widget. Real cell edits still commit minimal document transactions through the component’s `table.edit` path.
...
- Current source-fidelity guard depends on the component’s internal annotation value `table.format`; compatibility casting exists only at the table capability boundary. Every component upgrade must re-verify non-canonical load, source→live preview, undo depth, canonical table widgets, and real cell edit. Remove that internal boundary once upstream provides a public autoformat opt-out.
...
- If upstream provides a public API that both preserves original whitespace and can mount widgets, replace the current raw-source fallback for non-canonical tables; do not silently normalize user Markdown as a substitute.
