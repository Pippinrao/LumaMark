...
- 平台：Windows，本地开发工作树
- 命令：`pnpm perf:bench`
- 覆盖范围：Markdown fixture 读取、应用文件动作打开、打开后 debounce 大纲刷新、虚拟化大纲面板初始渲染、CodeMirror 大文档初始化、尾部输入 dispatch、selection-only dispatch、显示模式往返、代码块密集文档，以及简单/复杂 Mermaid pending render 输入 dispatch
...
| 路径 | 预算 | 当前结果 | 结论 |
|---|---:|---:|---|
| 读取 `large-1mb.md` | < 300 ms | 1.95 ms | 通过 |
| 读取 `large-5mb.md` | < 1000 ms | 5.85 ms | 通过 |
| 读取 `large-10mb.md` | < 2000 ms | 11.70 ms | 通过 |
| 文件动作打开 `large-1mb.md` | < 300 ms | 80.36 ms | 通过 |
| 文件动作打开 `large-5mb.md` | < 1000 ms | 107.93 ms | 通过 |
| 文件动作打开 `large-10mb.md` | < 2000 ms | 172.24 ms | 通过 |
| 打开后大纲刷新 `large-1mb.md` | < 50 ms | 7.38 ms | 通过 |
| 打开后大纲刷新 `large-5mb.md` | < 150 ms | 25.18 ms | 通过 |
| 打开后大纲刷新 `large-10mb.md` | < 300 ms | 48.91 ms | 通过 |
| 大纲面板初始渲染 `large-1mb.md` | < 60 ms | 23 / 799 项，15.12 ms | 通过 |
| 大纲面板初始渲染 `large-5mb.md` | < 60 ms | 23 / 3953 项，11.12 ms | 通过 |
| 大纲面板初始渲染 `large-10mb.md` | < 60 ms | 23 / 7892 项，15.62 ms | 通过 |
| 编辑器载入 `large-1mb.md` | < 300 ms | 119.98 ms | 通过 |
| 编辑器载入 `large-5mb.md` | < 1000 ms | 49.07 ms | 通过 |
| 编辑器载入 `large-10mb.md` | < 2000 ms | 68.73 ms | 通过 |
| 1MB 尾部输入 dispatch | < 16 ms | 8.52 ms | 通过 |
| 5MB 尾部输入 dispatch | < 50 ms | 2.36 ms | 通过 |
| 10MB 尾部输入 dispatch | < 100 ms | 1.71 ms | 通过 |
| Mermaid 渲染 pending 时普通输入 dispatch | < 50 ms | 3.38 ms | 通过 |
| 1MB 文档 12 次 selection-only dispatch | < 100 ms | 20.91 ms（平均 1.74 ms） | 通过 |
| 5MB 文档 12 次 selection-only dispatch | < 120 ms | 9.92 ms（平均 0.83 ms） | 通过 |
| 10MB 文档 12 次 selection-only dispatch | < 160 ms | 8.80 ms（平均 0.73 ms） | 通过 |
| 1MB 文档 source/live-preview 模式往返 | < 150 ms | 23.09 ms | 通过 |
| 5MB 文档 source/live-preview 模式往返 | < 300 ms | 20.06 ms | 通过 |
| 10MB 文档 source/live-preview 模式往返 | < 600 ms | 30.07 ms | 通过 |
| 2048 个 fenced blocks（0.46 MiB）载入 | < 300 ms | 23.86 ms | 通过 |
| 2048 个 fenced blocks 尾部输入 dispatch | < 16 ms | 4.19 ms | 通过 |
| 复杂 Mermaid pending 时主文档输入 dispatch | < 50 ms | 180 节点 / 17,348 bytes，3.34 ms | 通过 |
