...
- 1MB document-statistics real-document warmup: 2026-08-25
...
| 1MB document statistics (sync count) | P80 < 16 ms; max < 50 ms | 2026-08-22 serial `pnpm perf:bench`: P80 9.44 ms; max 12.53 ms; samples [12.53, 9.44, 6.66, 6.67, 6.73]. n=3 made P80 equal max and failed Windows CI on first sample 19.34 ms (warm 12.53 / 10.27). 2026-08-25 `main` 0.3.56 isolated `pnpm perf:bench`: P80 17.49 ms; max 20.79 ms; samples [20.79, 12.45, 10.56, 11.43, 17.49] after warming a one-line Text. Same commit on the PR gate: P80 9.28 ms; samples [12.70, 9.28, 6.72, 6.59, 6.58]. Warmup now uses the counted 1MB document; 16 ms P80 is unchanged. | Pass |
| 5MB document statistics schedule (input path) | P80 < 2 ms; max < 50 ms | 2026-08-22 serial `pnpm perf:bench`: P80 0.09 ms; max 0.18 ms; samples [0.18, 0.02, 0.02, 0.01, 0.09] | Pass |
| 10MB document statistics schedule (input path) | P80 < 2 ms; max < 50 ms | 2026-08-22 serial `pnpm perf:bench`: P80 0.01 ms; max 0.03 ms; samples [0.03, 0.01, 0.01, 0.00, 0.00] | Pass |
...
| 32 everyday GFM tables, Adaptive page width, editor create | P80 < 300 ms; max < 600 ms | 2026-08-25 serial `pnpm perf:bench`: P80 137.88 ms; max 318.55 ms; samples [318.55, 137.88, 113.85, 97.84, 100.05] | Pass |
| 32 everyday GFM tables, Standard page width, editor create | P80 < 300 ms; max < 600 ms | 2026-08-25 serial `pnpm perf:bench`: P80 107.38 ms; max 116.67 ms; samples [99.41, 105.88, 116.67, 104.21, 107.38] | Pass |
...
- 0.3.52 did not enforce a many-table open gate. The mixed-document and installed stutter fixtures mount one everyday `.tbl-table-widget`. Issue #28 (opening multi-table files stalls even with Adaptive off) came from forcing table widgets to the track width and disabling `contain: paint`, which is independent of the page-width preset. The 2026-08-25 gate opens 32 everyday GFM tables at Adaptive and Standard; `pnpm perf:bench` is still required and is not skipped.
- 1MB document-statistics sync count now uses ADR 0007 five-sample P80 and a 50 ms hard max. Collecting only three samples made P80 equal the maximum, so one Windows JIT/GC hitch (19.34 ms) failed a 16 ms primary budget while the two warm samples stayed at 12.53 / 10.27 ms. Warming a one-line `Text` still left the first 1MB sample cold: `main` 0.3.56 isolated `pnpm perf:bench` measured [20.79, 12.45, 10.56, 11.43, 17.49] (P80 17.49 ms) while the same commit’s PR gate measured P80 9.28 ms. Warmup now runs `getDocumentStatisticsFromText` on the counted 1MB document. The 16 ms P80 budget is unchanged.
