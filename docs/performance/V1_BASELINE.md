...
- Many-table file-open gate date: 2026-08-25
...
| 1MB document statistics (sync count) | P80 < 16 ms; max < 50 ms | 2026-08-22 serial `pnpm perf:bench`: P80 9.44 ms; max 12.53 ms; samples [12.53, 9.44, 6.66, 6.67, 6.73]. n=3 made P80 equal max and failed Windows CI on first sample 19.34 ms (warm 12.53 / 10.27). | Pass |
...
- 1MB document-statistics sync count now uses ADR 0007 five-sample P80 and a 50 ms hard max. Collecting only three samples made P80 equal the maximum, so one Windows JIT/GC hitch (19.34 ms) failed a 16 ms primary budget while the two warm samples stayed at 12.53 / 10.27 ms. The 16 ms P80 budget is unchanged.
