> Language: **English** · [中文](../zh/product/V1_UX_DESIGN.md)
...
- Default **file management mode**: one sidebar (Files / Outline tabs) + one central editor; no permanent three-column outline.
- Desktop **menu bar** (File, Edit, Paragraph, Format, View, Theme, Help)—not a heavy toolbar.
- Visual direction: quiet native workspace, restrained chrome, paper-grade editor; light clean / dark low-glare; no decorative “marketing hero” first screen.
- Alpha used a hi-fi Web prototype under `docs/zh/product/prototypes/v1-apple-file-mode/` as a pre-implementation review gate; that prototype is historical visual regression material, not the production app entry.

Later work should still respect durable layout and visual principles here, but outdated “pre-implementation” prototype steps must not override current code, tests, or the active plan.
