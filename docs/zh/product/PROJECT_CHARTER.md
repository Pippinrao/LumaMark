> 语言：**中文** · [English](../../product/PROJECT_CHARTER.md)

# LumaMark 项目章程

日期：2026-07-04

## 一句话定位

LumaMark 是一个高性能、现代化、跨平台的所见即所得 Markdown 编辑器，第一阶段以 WYSIWYG 体验为基线，先复刻成熟写作体验，再在性能、工程能力和现代工作流上创新。

## 愿景

成为世界第一的所见即所得 Markdown 编辑器。

这里的“世界第一”不是功能数量最多，而是在以下方面做到综合领先：

- 写作体验自然、安静、稳定。
- 大文档依然流畅。
- Markdown 源码保真。
- Mermaid、公式、图片、代码块等高成本内容不阻塞输入。
- 界面现代、美观、轻量。
- 多语言和跨平台体验可靠。
- AI 原生开发流程让项目长期可维护。

## 使命

帮助用户用 Markdown 进行长时间、低干扰、高可靠的写作和文档维护。

LumaMark 要让用户感觉：

- 打开快。
- 输入顺。
- 滚动稳。
- 保存可信。
- 界面舒服。
- 重文档也不怕。

## 战略路线

LumaMark 采用“先复刻，再创新”的路线。

### 先复刻

第一阶段不急于发散创新，先对齐成熟的 WYSIWYG 范式：

- 中央编辑区是第一主角。
- 侧边栏和大纲可选、克制。
- Markdown 源码符号在合适时机隐藏或弱化。
- 编辑和阅读融为一体。
- 常用 Markdown 输入动作顺滑可预测。

复刻的目标是降低用户学习成本，并建立一个清晰、可比较的体验基线。

### 再创新

在基线稳定后，创新集中在 LumaMark 的差异化方向：

- 大文档性能。
- 源码保真。
- 异步块渲染。
- 现代 UI 和主题系统。
- 工作区、全文搜索和命令面板。
- 可扩展能力。
- AI 辅助写作和 Markdown 重构。

## 目标用户

核心用户：

- 开发者
- 技术作者
- 产品经理
- 研究者
- 写作型知识工作者
- 长期使用 MarkText、Obsidian、Zettlr 等 Markdown 工具的用户

这些用户共同关心：

- 写作时不被 UI 打扰。
- Markdown 文件可长期保存和迁移。
- 大文档不能卡。
- 编辑器不能悄悄改坏文件。
- Mermaid、代码块、图片、目录等内容必须可靠。

## 成功标准

短期成功：快速超过 MarkText。

- 更流畅。
- 更现代。
- 源码更可信。
- 维护策略更清晰。
- Windows 体验更稳定。

中期成功：完善核心写作体验。

- 其他 Markdown 编辑器用户能无明显阻力迁移。
- 常用 Markdown 编辑动作自然。
- 图片、链接、表格、代码块、Mermaid、公式、导出等核心能力完整。

长期成功：发展独立的产品能力。

- 大文档体验明显更好。
- 工作区和搜索能力更强。
- 源码保真更可靠。
- UI 更现代。
- 性能观测、自动化质量和扩展能力更强。

## 非目标

第一阶段不做：

- 云同步平台。
- 完整插件市场。
- 团队协作套件。
- 类 Notion 数据库。
- 复杂知识图谱。
- 移动端应用。
- 学术写作全家桶。

这些能力可以在核心编辑体验稳定后重新评估。

## 核心约束

- 优先使用成熟组件。
- 不手搓基础组件，除非用户明确批准。
- Markdown 源文件是唯一真实数据。
- 所有 UI 文案必须可本地化。
- 编辑器热路径不交给 React。
- 性能指标必须自动化测量。
- AI 开发必须遵守测试、验证和审查门禁。
