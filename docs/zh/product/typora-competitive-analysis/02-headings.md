> 语言：**中文** · [English](../../../product/typora-competitive-analysis/02-headings.md)

# 标题（Headings）竞品差距分析

> **菜单系统实施更新（2026-08-02）：** 顶部“段落”菜单现在直接提供标题 1–6，菜单显示并实际执行 `Ctrl+1…6`；新增“普通段落”命令使用 `Ctrl+0`，只移除当前 ATX heading marker，并保持缩进、正文和一次撤销语义。`Ctrl+0` 是 LumaMark 明确的归一化命令，不冒充 Typora 已核实键位。菜单、命令面板和 CodeMirror keymap 共用同一编辑器 command port；这只补齐入口一致性，不改变本文对逐键创建、IME、边界保真和可访问语义仍有差距的判断。
