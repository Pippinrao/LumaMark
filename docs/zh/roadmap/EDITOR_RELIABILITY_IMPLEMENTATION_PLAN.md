> 语言：**中文** · [English](../../roadmap/EDITOR_RELIABILITY_IMPLEMENTATION_PLAN.md)

﻿# 编辑器可靠性实施计划

> **状态：当前执行计划**
>
> 本文是 LumaMark 当前唯一的实施路线事实来源。Foundation 与 MarkText+ 已形成技术基线；当前里程碑进入 **Parity Reliability Foundation**，目标是让已有能力达到可长期日用的 WYSIWYG 可靠性，而不是继续按专题堆叠功能。
...
LumaMark 独立定义写作流程，以及源码保真、安全与性能合同。相关架构决策见 [ADR 0006](../decisions/0006-parity-reliability-editor-contracts.md)。
...
完整命令与完成定义遵循 [DEVELOPMENT_PROCESS.md](../../../DEVELOPMENT_PROCESS.zh.md) 和 [质量策略](../quality/QUALITY_STRATEGY.md)。性能事实与预算以 `docs/zh/performance/` 下对应基准文档为准。
...
2. **剪贴板合同**
   - 复制为纯文本、Copy as Markdown、`Ctrl+Shift+V` 粘贴为纯文本；随后再接到右键与顶栏。
...
4. **表格行列、对齐、粘贴合同与组件内菜单双语化**
   - 统一 `Ctrl+L` / `Ctrl+E` / 删行等语义；`codemirror-markdown-tables` 内文案进 i18n（发布阻断项）。
